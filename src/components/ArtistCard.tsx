...
const ArtistCard: React.FC<ArtistCardProps> = ({ artist, onClick }) => {
  ...
  if (!hasImage) {
    return (
      <MotionDiv
        className="group relative h-[300px] md:h-[360px] w-full overflow-hidden border border-white/10 bg-[#0f172a] hover:bg-[#1e293b] cursor-pointer rounded-xl flex flex-col items-center justify-center p-8 text-center transition-colors"
        initial="rest"
        whileHover="hover"
        whileTap="hover"
        animate="rest"
  ...
  return (
    <MotionDiv
      className="group relative h-[300px] md:h-[360px] w-full overflow-hidden border border-white/10 bg-[#0f172a] cursor-pointer rounded-xl"
      initial="rest"
      whileHover="hover"
      whileTap="hover"
      animate="rest"
